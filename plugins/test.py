...
def health_check(uid):
    ...
    if v == 0:
        ...
        if pp < 300:
            ...
            level = "结论:基本断定是小号或者离线党!"
        elif A3 < 1.7: 
            if A1 < 9:
                level = "结论:要么天赋超群，要么小号或者离线党,总之这个pp严重虚低!"
            else:
                if A4 < 0.75:
                    level = "结论:虽然天赋超群,但是求你别糊图了!"
                elif A4 < 0.88: 
                    level = "结论:虽然天赋超群，但是建议花些pc好好练习一下acc吧!"
                elif A2 < 1.7: 
                    level = "结论:是一个有天赋的超级pp刷子,求求你不要re了!"
                elif A2 < 1.9: 
                    level = "结论:是一个有天赋的高级pp刷子,建议降低re图次数!"
                else:
                    level = "结论:是一个有天赋又认真的pp刷子,建议多打点综合图!"
            
        
        elif A3 < 1.9:
            if A1 < 9 and A4 > 0.75: 
                level = "结论:有一定天赋，将来一定时间内还是可以飞升一波的!"
            if A1 < 11 and A4 > 0.75: 
                level = "结论:有一定天赋，将来一定时间内还是可以小幅涨一点的!"
            else:
                if A4 < 0.75: 
                    level = "结论:虽然有一些天赋,但是求你别糊图了!"
                elif A4 < 0.88: 
                    level = "结论:虽然有一些天赋，但是建议花些pc好好练习一下acc吧!"
                elif A2 < 1.7: 
                    level = "结论:是一个标准pp刷子,求求你不要re了!"
                elif A2 < 1.9: 
                    level = "结论:是一个标准pp刷子,建议降低re图次数!"
                elif A2 < 2.1: 
                    level = "结论:是一个标准pp刷子,建议多打点综合图!"
                else: 
                    level = "结论:这种情况比较罕见，你应该和各种类型的人都不一样!"
            
        
        elif A3 < 2.1:
            if A1 < 9 and A4 > 0.75: 
                level = "结论:看样子正渡过瓶颈期了，将来一定时间内还是可以飞升一波的!"
            if A1 < 11 and A4 > 0.75: 
                level = "结论:要么即将渡过瓶颈期，要么之前飞太快即将进入瓶颈期!"
            else:
                if A4 < 0.75: 
                    level = "结论:你啥都不错,但是求你别糊图了!"
                elif A4 < 0.88: 
                    level = "结论:比较正常，但是建议好好练习一下acc吧!"
                elif A2 < 1.7: 
                    level = "结论:是一个没天赋的pp刷子,求求你不要re了!"
                elif A2 < 1.9: 
                    level = "结论:是一个没天赋的pp刷子,建议降低re图次数!"
                else:
                    level = "结论:比较正常，但是可能某些方面有所欠缺，请参考指标!"
            ...
            if A1 < 9 and A4 > 0.75: 
                level = "结论:相信自己，你正在飞升!"
            if A1 < 11 and A4 > 0.75: 
                level = "结论:也许在瓶颈期附近，但是相信你能克服它!"
            else:
                if A4 < 0.75: 
                    level = "结论:你真的很强,但是求你别糊图了!"
                elif A4 < 0.88: 
                    level = "结论:你真的很强，但是建议好好练习一下acc吧!"
                elif A2 < 1.7: 
                    level = "结论:是一个没救了的pp刷子,求求你不要re了!"
                elif A2 < 1.9: 
                    level = "结论:是一个没救了的pp刷子,建议降低re图次数!"
                else:
                    level = "结论:这孩子瓶颈了!"
            
        else:
            if A1 < 10 and A4 > 0.75: 
                level = "结论:打图经验充足，不飞升没理由!"
            else:
                if A2 < 1.8: 
                    level = "结论:你这么个re图毫无用处，好好考虑下吧!"
                else:
                    level = "结论:你不适合屙屎，删游戏吧!"
